...
- [추가 주제 46-61 (모바일 플랫폼·AI·파이프라인 보강)](#추가-주제-46-61) — 46~61
- [추가 주제 62-77 (3D 그래픽스·게임 AI·플랫폼 확장)](#추가-주제-62-77) — 62~77
...
> 2025-08(Unity 6.2)부터 Muse·Sentis가 **"Unity AI"** 브랜드로 통합·재편됐다. 추론 엔진 패키지(`com.unity.ai.inference`)는 유지되지만, 콘텐츠 생성 쪽은 → 69. 생성형 AI 제작 툴에서 다룬다.
...
기존 주제에서 약했던 **3D 그래픽스·씬 최적화**, **게임플레이 AI·시스템**, **플랫폼 확장(AR/XR·WebGL)**을 보강한 주제다. 선행 학습 자료의 URL은 2026-06 기준 공식 문서를 확인해 채웠고, 서드파티 도구(FMOD·Wwise·Yarn Spinner 등)는 공식 패키지가 아니므로 도입 시 라이선스·버전 확인이 필요하다.

### 62. 라이팅·GI·라이트맵·라이트 프로브 〔그래픽스·애니메이션·사운드〕

**목적·목표**

- **목적**: 베이크 전역 조명(GI)·라이트맵·라이트 프로브로 3D 씬 조명을 굽고, 정적/동적 오브젝트의 조명 차이를 다룰 수 있게 된다.
- **목표**
  1. Light Mode(Realtime/Mixed/Baked)와 정적 플래그를 이해하고 라이트맵을 굽는다.
  2. 라이트 프로브·리플렉션 프로브로 동적 오브젝트의 간접광·반사를 근사한다.
  3. 라이트맵 UV·해상도·용량과 모바일에서의 베이크 비용·품질 절충을 정리한다.

**핵심 용어**: 전역 조명(GI, 간접광까지 포함한 조명 계산), 라이트맵(미리 구운 조명을 담은 텍스처), 라이트 프로브(공간의 조명을 샘플링해 동적 오브젝트에 근사 적용).

**선행 학습 자료**

| 자료 | 출처 |
|---|---|
| Introduction to lightmaps and baking | https://docs.unity3d.com/6000.2/Documentation/Manual/Lightmappers.html |
| Light Modes | https://docs.unity3d.com/6000.0/Documentation/Manual/LightModes-introduction.html |
...
- **다룰 내용**: Light Mode·정적 플래그, 라이트맵 베이크·UV·해상도, 라이트/리플렉션 프로브, 모바일 베이크 비용.
- **다루지 않을 내용**: 후처리·2D 라이팅(→ 51), 셰이더 작성(→ 14, → 19), 배칭(→ 29).

**산출물**: 라이트맵 베이크 씬과 Light Mode 정리, 프로브로 동적 오브젝트 조명 근사 예제, 라이트맵 용량·품질 절충 노트.

### 63. 오클루전 컬링·밉맵 스트리밍·LOD 〔성능·메모리〕

**목적·목표**

- **목적**: 안 보이는 것을 안 그리고(오클루전 컬링), 텍스처 메모리를 예산 내로 스트리밍하고, 거리별 LOD로 3D 씬의 렌더·메모리 비용을 다룰 수 있게 된다.
- **목표**
  1. 오클루전 컬링을 베이크하고 카메라가 가려진 오브젝트를 제외하는지 확인한다.
  2. Mipmap Streaming으로 카메라 위치에 필요한 밉맵만 로드해 GPU 메모리를 예산 내로 유지한다.
  3. LOD Group으로 거리별 메시 단계를 두고, 컬링·스트리밍·LOD 적용 전후 비용을 비교한다.

**핵심 용어**: 오클루전 컬링(다른 물체에 가려 안 보이는 것을 그리지 않기), 밉맵 스트리밍(필요한 해상도 밉맵만 메모리에 올리기), LOD(거리에 따라 디테일이 낮은 메시로 교체).
...
| Set up a scene for occlusion culling | https://docs.unity3d.com/6000.4/Documentation/Manual/occlusion-culling-getting-started.html |
| Optimizing GPU texture memory with mipmap streaming | https://docs.unity3d.com/Manual/TextureStreaming.html |
| Level of Detail (LOD) | https://docs.unity3d.com/Manual/LevelOfDetail.html |

**리서치 범위**

- **다룰 내용**: 오클루전 컬링 베이크, 밉맵 스트리밍 예산, LOD Group, 적용 전후 렌더·메모리 비교.
- **다루지 않을 내용**: 드로우콜 배칭(→ 29), 에셋 메모리 일반(→ 05), Addressables 수명(→ 07).

**산출물**: 오클루전 컬링·LOD 적용 씬, 밉맵 스트리밍 예산 설정 노트, 컬링/스트리밍/LOD 적용 전후 비용 비교.

### 64. 셰이더 베리언트·프리워밍 〔성능·메모리〕

**목적·목표**

- **목적**: 셰이더 변형을 처음 쓸 때 발생하는 컴파일 끊김(히칭)을 베리언트 수집·프리워밍으로 로딩 중에 미리 해소할 수 있게 된다.
- **목표**
  1. 셰이더 베리언트와 PSO(파이프라인 상태 객체)가 첫 사용 시 컴파일되며 끊김을 만드는 과정을 재현한다.
  2. 플레이 세션을 ShaderVariantCollection으로 녹화하고 Preloaded Shaders·`WarmUp()`으로 로딩 중에 미리 컴파일한다.
  3. 빌드 시 스트리핑으로 누락되는 변형과 baseline 보존(link/keyword) 정책을 정리한다.

**핵심 용어**: 셰이더 베리언트(키워드 조합마다 만들어지는 셰이더 변형), 프리워밍(첫 사용 전에 미리 컴파일해 드라이버 캐시에 올리기), 스트리핑(빌드에서 쓰지 않는 변형 제거 — 누락 주의).

**선행 학습 자료**

| 자료 | 출처 |
|---|---|
| Prewarm shaders / Warm up PSOs | https://docs.unity3d.com/6000.3/Documentation/Manual/shader-prewarm.html |
| Create a shader variant collection | https://docs.unity3d.com/Manual/shader-variant-collections.html |
| ShaderVariantCollection.WarmUp (API) | https://docs.unity3d.com/ScriptReference/ShaderVariantCollection.WarmUp.html |
| Shader variants optimization & troubleshooting | https://unity.com/blog/engine-platform/shader-variants-optimization-troubleshooting-tips |

**리서치 범위**

- **다룰 내용**: 베리언트·PSO 컴파일 히칭 재현, SVC 녹화·Preloaded·WarmUp, 스트리핑 누락·baseline 정책.
- **다루지 않을 내용**: 셰이더 작성 자체(→ 14, → 19), 코드 스트리핑(→ 31), 배칭(→ 29).

**산출물**: 셰이더 히칭 재현과 프리워밍 적용 전후 비교, SVC 녹화·WarmUp 코드, 베리언트 스트리핑 정책 노트.

### 65. Unity 6 렌더링 신기능(GPU Resident Drawer·Render Graph) 〔성능·메모리〕

**목적·목표**

- **목적**: Unity 6의 GPU Resident Drawer·GPU 오클루전 컬링·Render Graph로 CPU 렌더 부하를 줄이고, 활성화 조건과 효과를 측정할 수 있게 된다.
- **목표**
  1. Forward+·SRP Batcher·BatchRendererGroup 조건을 맞추고 GPU Resident Drawer를 켜 드로우 비용 변화를 측정한다.
  2. GPU 오클루전 컬링을 켜 이전 프레임 깊이로 가려진 오브젝트를 제외한다.
  3. Render Graph(Compatibility Mode off)의 패스 구성·리소스 관리 개념을 확인한다.

**핵심 용어**: GPU Resident Drawer(인스턴싱을 GPU에서 자동 처리해 CPU 부하를 줄이는 기능), GPU 오클루전 컬링(GPU에서 깊이 기반으로 가려진 것 제외), Render Graph(렌더 패스·리소스를 그래프로 관리하는 URP의 새 구조).

**선행 학습 자료**

| 자료 | 출처 |
|---|---|
| Enable the GPU Resident Drawer in URP | https://docs.unity3d.com/6000.0/Documentation/Manual/urp/gpu-resident-drawer.html |
| Enable GPU occlusion culling in URP | https://docs.unity3d.com/6000.0/Documentation/Manual/urp/gpu-culling.html |
| Render Graph in URP | https://docs.unity3d.com/6000.0/Documentation/Manual/urp/render-graph.html |

**리서치 범위**

- **다룰 내용**: GPU Resident Drawer 조건·효과, GPU 오클루전 컬링, Render Graph 개념, 적용 전후 측정.
- **다루지 않을 내용**: 전통적 배칭(→ 29), CPU 오클루전 컬링 베이크(→ 63), 커스텀 포스트 프로세스(→ 51).

**산출물**: GPU Resident Drawer·오클루전 켜고 끈 드로우/프레임 비교, Render Graph 패스 구성 메모, 활성화 조건 체크리스트.

### 66. 오디오 미들웨어(FMOD·Wwise) 〔그래픽스·애니메이션·사운드〕

**목적·목표**

- **목적**: 프로덕션 표준 오디오 미들웨어(FMOD·Wwise)를 연동해 이벤트 기반 사운드·동적 믹스를 다루고, Unity 내장 AudioMixer와의 선택 기준을 세울 수 있게 된다.
- **목표**
  1. FMOD 또는 Wwise를 임포트하고 이벤트·파라미터·버스로 사운드를 재생·제어한다.
  2. 게임플레이 파라미터(거리·체력·강도)에 따른 동적 믹스·전환을 구현한다.
  3. 내장 AudioMixer 대비 워크플로·기능·용량·러닝커브를 비교해 도입 기준을 정리한다.

**핵심 용어**: 오디오 미들웨어(엔진과 사운드 에셋 사이의 전문 오디오 계층), 이벤트(미들웨어에서 정의한 재생 단위), 파라미터(게임 값으로 사운드를 실시간 제어).

> FMOD·Wwise는 Unity 공식 패키지가 아니다. 도입 시 라이선스(매출 구간별)·버전 고정·플랫폼 빌드 설정을 별도로 확인한다.

**선행 학습 자료**

| 자료 | 출처 |
|---|---|
| FMOD for Unity (Welcome) | https://www.fmod.com/docs/2.03/unity/welcome.html |
| fmod/fmod-for-unity (저장소) | https://github.com/fmod/fmod-for-unity |
| Wwise Unity Integration | https://www.audiokinetic.com/library/edge/?id=pg__appscripts.html |
| Unity Audio overview(내장 대비) | https://docs.unity3d.com/Manual/Audio.html |

**리서치 범위**

- **다룰 내용**: 미들웨어 이벤트·파라미터·버스, 동적 믹스, AudioMixer 대비 선택 기준.
- **다루지 않을 내용**: 내장 AudioMixer 기초(→ 25), 오디오 에셋 Addressables 수명(→ 07).
- **연계**: 25. 오디오 시스템과 AudioMixer

**산출물**: FMOD/Wwise 이벤트·파라미터 연동 데모, 동적 믹스 샘플, AudioMixer vs 미들웨어 선택 기준 표.

### 67. AI 행동 설계(Unity Behavior·Behavior Tree·Utility AI) 〔게임플레이·데이터〕

**목적·목표**

- **목적**: NavMesh 이동·FSM 위에 "무엇을 할지" 결정하는 의사결정 AI를, Behavior Tree·Utility AI로 설계할 수 있게 된다.
- **목표**
  1. Unity Behavior(behavior graph)로 순찰·추적·공격·후퇴 같은 모듈형 행동을 구성한다.
  2. Blackboard·조건·서브그래프로 상태 공유와 재사용을 다룬다.
  3. Behavior Tree와 Utility AI(점수 기반 선택)의 차이와 적합한 상황을 정리한다.

**핵심 용어**: Behavior Tree(노드 계층으로 행동을 선택·조합하는 구조), Blackboard(에이전트가 공유하는 데이터 저장소), Utility AI(각 행동에 점수를 매겨 가장 높은 것을 고르는 방식).

**선행 학습 자료**

| 자료 | 출처 |
|---|---|
| Unity Behavior 패키지 | https://docs.unity3d.com/Packages/com.unity.behavior@1.0/manual/index.html |
| Behavior graphs | https://docs.unity3d.com/Packages/com.unity.behavior@1.0/manual/behavior-graph.html |
| Behavior (Unity Manual) | https://docs.unity3d.com/6000.2/Documentation/Manual/com.unity.behavior.html |
| 『게임 프로그래밍 패턴』(State 등) | https://gameprogrammingpatterns.com/ |

**리서치 범위**

- **다룰 내용**: Behavior graph 행동 구성, Blackboard·조건·서브그래프, BT vs Utility AI 선택.
- **다루지 않을 내용**: 경로 탐색·이동(→ 38), 애니메이션 상태(→ 10), ML 기반 학습(→ 50).
- **연계**: 10. Animator와 State Machine, 38. NavMesh와 AI 내비게이션

**산출물**: Behavior graph로 만든 적 AI 데모, Blackboard·서브그래프 재사용 예제, BT vs Utility 선택 기준 노트.

### 68. 스킬·어빌리티 시스템 아키텍처 〔게임플레이·데이터〕

**목적·목표**

- **목적**: 전투 스킬·버프·상태이상·효과를 데이터 기반으로 조합하는 확장 가능한 어빌리티 시스템을 설계할 수 있게 된다.
- **목표**
  1. 스킬을 "효과(Effect)의 조합"으로 보고, 데미지·힐·이동·상태이상을 데이터로 정의한다.
  2. 버프/디버프·지속효과·쿨다운·코스트를 시간 축에서 관리하는 구조를 만든다.
  3. 새 스킬·새 효과 추가 시 수정 범위를 최소화하는 설계(데이터+조합)를 검증한다.

**핵심 용어**: 어빌리티(스킬, 효과들의 조합으로 실행되는 행동), 효과(Effect, 데미지·상태이상 등 가장 작은 단위), 버프/디버프(일정 시간 능력치·상태를 바꾸는 지속 효과).
...
|---|---|
| ScriptableObject architecture eBook | https://unity.com/resources/create-modular-game-architecture-with-scriptable-objects-ebook |
| 『게임 프로그래밍 패턴』(Component·Type Object) | https://gameprogrammingpatterns.com/ |
| ScriptableObject Manual | https://docs.unity3d.com/Manual/class-ScriptableObject.html |

**리서치 범위**

- **다룰 내용**: 효과 단위 분해, 스킬=효과 조합, 버프·지속효과·쿨다운·코스트, 확장성 검증.
- **다루지 않을 내용**: 데이터 테이블 임포트(→ 49), 인벤토리/아이템 설계(→ 13), 이벤트 통신(→ 54), 연출(→ 15).
- **연계**: 13. 인벤토리 설계와 ScriptableObject, 49. 게임 데이터 테이블 파이프라인, 54. 이벤트 버스·메시징

**산출물**: 효과 조합형 스킬 시스템 샘플, 버프·쿨다운·코스트 관리 코드, 새 스킬 추가 영향 측정 노트.

### 69. 생성형 AI 제작 툴(Unity AI·Muse 후속) 〔AI·머신러닝〕

**목적·목표**

- **목적**: 에디터 통합 생성형 AI(Unity AI / 구 Muse)로 스프라이트·텍스처·머티리얼·애니메이션·사운드를 생성해 1인 개발의 콘텐츠 병목을 줄이는 활용·한계를 다룰 수 있게 된다.
- **목표**
  1. Unity AI Generators로 스프라이트·텍스처·머티리얼·애니메이션 시안을 생성한다.
  2. 생성물의 일관성·저작권·라이선스(서드파티 모델 의존)와 프로덕션 반입 기준을 정리한다.
  3. 에디터 내 Chat·추천 기능을 보조 도구로 활용하는 워크플로를 정한다.

**핵심 용어**: Unity AI(에디터 통합 AI 도구 모음 — Muse·Sentis 후속), Generators(스프라이트·텍스처·사운드 등을 생성하는 도구), 시안(production 반입 전의 빠른 초안).

> Unity AI는 빠르게 변하는 영역이고 일부 기능은 유료·서드파티 모델 의존이다. 생성물의 상업적 사용·저작권 조건을 도입 시 반드시 확인한다.

**선행 학습 자료**

| 자료 | 출처 |
|---|---|
| Unity Muse capabilities in Editor (블로그) | https://unity.com/blog/engine-platform/unity-muse-ai-capabilities-in-editor-plus-new-updates |
| Unity AI in Unity 6.2 (뉴스) | https://www.cgchannel.com/2025/08/unity-rolls-out-unity-ai-in-unity-6-2/ |
| Prototype a game with Unity Muse AI (Learn) | https://learn.unity.com/project/prototype-a-game-with-unity-muse-AI |

**리서치 범위**

- **다룰 내용**: 스프라이트·텍스처·머티리얼·애니메이션 생성, 저작권·라이선스·반입 기준, 보조 활용 워크플로.
- **다루지 않을 내용**: 온디바이스 추론·학습(→ 50), 콘텐츠 임포트 파이프라인(→ 43, → 72).
- **연계**: 50. Unity Sentis·ML-Agents, 43. 콘텐츠·아트 파이프라인

**산출물**: 생성형 AI로 만든 에셋 시안과 반입 결과, 저작권·라이선스 점검 노트, AI 보조 제작 워크플로 메모.

### 70. 멀티플레이어 심화(예측·보간·롤백·전용 서버) 〔출시·운영·네트워크〕

**목적·목표**

- **목적**: 지연·끊김 환경에서도 매끄러운 동기화를 위해 클라이언트 예측·보간·서버 권위·전용 서버 호스팅을 다룰 수 있게 된다.
- **목표**
  1. Network Simulator로 지연·손실을 넣고, 보간·클라이언트 예측·서버 재조정(reconciliation)을 적용한다.
  2. 서버 권위 판정과 클라이언트 예측의 경계를 정해 치트·불일치를 줄인다.
  3. 전용 서버(DGS) 호스팅과 P2P(Relay)·중계형의 선택 기준을 정리한다.

**핵심 용어**: 클라이언트 예측(입력 결과를 서버 확인 전에 미리 표시), 서버 재조정(서버 결과로 클라이언트 상태 보정), 전용 서버(DGS, 권위 판정을 담당하는 서버 인스턴스).
...
|---|---|
| Netcode for GameObjects | https://docs-multiplayer.unity3d.com/netcode/current/about/ |
| Multiplayer Tools — Network Simulator | https://docs-multiplayer.unity3d.com/tools/current/tools-network-simulator/ |
| Game Server Hosting (Multiplay) | https://docs.unity.com/ugs/manual/game-server-hosting/manual/welcome |
| Photon Fusion(대안) | https://doc.photonengine.com/fusion/current/getting-started/fusion-intro |

**리서치 범위**

- **다룰 내용**: 지연·손실 시뮬, 보간·예측·재조정, 서버 권위 경계, 전용 서버 vs Relay 선택.
- **다루지 않을 내용**: 연결·복제 기초(→ 33), Relay·Lobby·Matchmaker 구성(→ 53), 안티치트 일반(→ 35).
- **선행**: 33. 멀티플레이어·네트워킹 기초, 53. 멀티플레이어 인프라

**산출물**: 예측·보간·재조정 적용 데모, 서버 권위 경계 노트, 전용 서버 vs Relay 선택 기준 표.

### 71. UPM 패키지·모듈화·컴파일 시간 최적화 〔아키텍처·테스트·툴링〕

**목적·목표**

- **목적**: 코드를 사내 UPM 패키지·어셈블리로 모듈화해 재사용성과 컴파일/빌드 시간을 개선할 수 있게 된다.
- **목표**
  1. Assembly Definition으로 모듈을 분리하고, 변경 시 재컴파일 범위가 어떻게 줄어드는지 측정한다.
  2. 로컬·Git·레지스트리 기반 커스텀 UPM 패키지를 만들어 여러 프로젝트에서 재사용한다.
  3. 순환 의존·과도한 참조를 점검하고 모듈 경계·컴파일 시간 개선을 정리한다.

**핵심 용어**: UPM(Unity Package Manager, 패키지로 코드·에셋을 관리), Assembly Definition(asmdef, 컴파일 단위를 나누는 정의), 모듈 경계(서로 의존을 제한해 변경 영향을 가두는 선).

**선행 학습 자료**

| 자료 | 출처 |
|---|---|
| Packages (Package Manager) | https://docs.unity3d.com/Manual/Packages.html |
...
| Assembly Definition & script compilation | https://docs.unity3d.com/Manual/ScriptCompilationAssemblyDefinitionFiles.html |

**리서치 범위**

- **다룰 내용**: asmdef 모듈 분리·재컴파일 범위, 커스텀 UPM 패키지, 의존 점검·컴파일 시간 개선.
- **다루지 않을 내용**: 빌드/테스트 CI(→ 20), DI 구조(→ 32), 템플릿 구성(→ 00).
- **연계**: 00. Lab 템플릿, 32. 아키텍처와 의존성 주입(DI)

**산출물**: asmdef 모듈화 전후 컴파일 시간 비교, 재사용 가능한 커스텀 UPM 패키지, 모듈 경계·의존 점검 노트.

### 72. 메시·모델·애니메이션 임포트(FBX·리깅·블렌드셰이프) 〔그래픽스·애니메이션·사운드〕

**목적·목표**

- **목적**: FBX 모델의 임포트 설정·리깅·블렌드셰이프를 이해하고, 메시·애니메이션을 런타임 비용을 고려해 들여올 수 있게 된다.
- **목표**
  1. Model/Rig/Animation 탭의 임포트 설정(스케일·노멀·리그 타입·아바타)을 다룬다.
  2. 블렌드셰이프(모핑)·스킨드 메시·본 수가 메모리·성능에 주는 영향을 확인한다.
  3. 메시 압축·노멀/탄젠트·리드/라이트 설정 등 모바일 임포트 규격을 정리한다.

**핵심 용어**: FBX(주 모델 교환 포맷), 리그/아바타(본 계층을 애니메이션에 매핑하는 설정), 블렌드셰이프(정점을 변형해 표정·모핑을 만드는 방식 — SkinnedMeshRenderer 사용).

**선행 학습 자료**

| 자료 | 출처 |
|---|---|
| Importing a model | https://docs.unity3d.com/6000.3/Documentation/Manual/ImportingModelFiles.html |
| Model Import Settings reference | https://docs.unity3d.com/Manual/class-FBXImporter.html |
| Rig tab Import Settings reference | https://docs.unity3d.com/Manual/FBXImporter-Rig.html |
| Model tab (블렌드셰이프 등) | https://docs.unity3d.com/Manual/FBXImporter-Model.html |

**리서치 범위**

- **다룰 내용**: Model/Rig/Animation 임포트 설정, 블렌드셰이프·스킨드 메시·본 비용, 메시 압축·모바일 규격.
- **다루지 않을 내용**: 2D 스프라이트·아틀라스(→ 43, → 61), Spine(→ 17), Animator 상태(→ 10).
- **연계**: 43. 콘텐츠·아트 파이프라인, 10. Animator와 State Machine

**산출물**: 모델 임포트 규격 문서, 블렌드셰이프·스킨드 메시 비용 측정, 모바일 메시 임포트 체크리스트.

### 73. 프라이버시·동의·ATT 〔출시·운영·네트워크〕

**목적·목표**

- **목적**: iOS ATT·GDPR 동의 관리(CMP)를 이해하고, 추적 동의·개인정보 처리를 스토어 정책에 맞게 다룰 수 있게 된다.
- **목표**
  1. iOS App Tracking Transparency 권한 요청을 SDK 초기화 전에 올바른 순서로 띄운다.
  2. Google User Messaging Platform(UMP) 등 CMP로 GDPR 동의를 수집·저장한다.
  3. 동의 상태에 따른 광고·분석 SDK 동작 분기와 거부 시 대응을 정리한다.

**핵심 용어**: ATT(App Tracking Transparency, iOS의 앱 간 추적 동의 프레임워크), CMP(동의 관리 플랫폼), IDFA(iOS 광고 식별자 — ATT 동의 시에만 접근).
...
| ATT compliance (Unity) | https://docs.unity.com/ads/en-us/manual/ATTCompliance |
| Privacy & messaging — UMP (Google) | https://developers.google.com/admob/unity/privacy |

**리서치 범위**

- **다룰 내용**: ATT 요청 순서, UMP/CMP 동의 수집, 동의 상태별 SDK 분기·거부 대응.
- **다루지 않을 내용**: 어트리뷰션 SDK 연동(→ 60), 스토어 법무 일반(→ 44), 데이터 보호 기술(→ 35).
- **연계**: 44. 스토어 출시와 사업·법무, 60. UA·어트리뷰션·딥링크·인앱 리뷰

**산출물**: ATT·UMP 동의 흐름 데모, 동의 상태별 SDK 분기 코드, 프라이버시 점검 체크리스트.

### 74. AR Foundation·XR·visionOS(PolySpatial) 〔출시·운영·네트워크〕

**목적·목표**

- **목적**: AR Foundation·XR Interaction Toolkit로 AR/MR 기능을 구성하고, Apple Vision Pro(PolySpatial) 등 공간 컴퓨팅의 전제와 제약을 파악할 수 있게 된다.
- **목표**
  1. AR Foundation으로 평면 감지·앵커·라이트 추정 같은 기본 AR 기능을 구현한다.
  2. XR Interaction Toolkit로 AR 제스처·오브젝트 배치·조작을 다룬다.
  3. visionOS/PolySpatial의 렌더링·입력 모델 차이와 도입 요건(구독·플랫폼)을 정리한다.

**핵심 용어**: AR Foundation(여러 AR 플랫폼을 추상화한 Unity 프레임워크), XR Interaction Toolkit(VR/AR 상호작용 컴포넌트 모음), PolySpatial(visionOS의 공유 공간 렌더링을 지원하는 패키지).

> visionOS의 VR/MR 앱 빌드는 Unity Pro/Enterprise 구독이 필요하다. 도입 전 플랫폼·라이선스 요건을 확인한다.
...
| AR Foundation 패키지 | https://docs.unity3d.com/Packages/com.unity.xr.arfoundation@latest |
| XR Interaction Toolkit | https://docs.unity3d.com/Packages/com.unity.xr.interaction.toolkit@3.0/manual/index.html |
| PolySpatial visionOS | https://docs.unity3d.com/Packages/com.unity.polyspatial.visionos@1.1/manual/visionOSPlatformOverview.html |

**리서치 범위**

- **다룰 내용**: AR Foundation 평면·앵커·라이트 추정, XR Interaction Toolkit 제스처·배치, visionOS/PolySpatial 전제·제약.
- **다루지 않을 내용**: 모바일 2D/3D 게임 일반 주제, 내비게이션(→ 38), 입력 일반(→ 04).

**산출물**: AR 평면 감지·오브젝트 배치 데모, XR 상호작용 예제, visionOS/PolySpatial 도입 요건 정리.

### 75. WebGL·인스턴트 게임 빌드 〔출시·운영·네트워크〕

**목적·목표**

- **목적**: WebGL 빌드의 메모리·로딩·성능 제약을 이해하고, 웹/인스턴트 게임으로 배포할 때의 차이를 다룰 수 있게 된다.
- **목표**
  1. WebGL 빌드를 만들고 압축·로딩·메모리 설정(Memory Size·압축 포맷)을 다룬다.
  2. 모바일 네이티브 대비 스레딩·플러그인·입력·파일 접근의 제약을 정리한다.
  3. 초기 다운로드 용량·로딩 시간을 줄이는 전략(스트리밍·코드 스트리핑)을 적용한다.

**핵심 용어**: WebGL(브라우저에서 도는 Unity 빌드 타깃), 인스턴트 게임(설치 없이 바로 실행되는 웹 기반 게임), 힙 메모리(WebGL은 고정 힙 예산 제약이 큼).
...
| WebGL: memory considerations | https://docs.unity3d.com/Manual/webgl-memory.html |
| WebGL performance considerations | https://docs.unity3d.com/Manual/webgl-performance.html |

**리서치 범위**

- **다룰 내용**: WebGL 빌드·압축·메모리 설정, 플랫폼 제약, 다운로드·로딩 최적화.
- **다루지 않을 내용**: 모바일 빌드(→ 31), 콜드 스타트 일반(→ 58), Addressables(→ 07).
- **연계**: 31. IL2CPP·빌드·앱 용량 최적화, 58. 콜드 스타트·앱 기동 시간 최적화

**산출물**: WebGL 빌드와 메모리·압축 설정 노트, 플랫폼 제약 정리, 다운로드·로딩 최적화 전후 비교.

### 76. 콘텐츠 시스템(퀘스트·미션·대화) 〔게임플레이·데이터〕

**목적·목표**

- **목적**: 퀘스트·미션·대화 같은 콘텐츠를 코드와 분리해 데이터로 정의·진행·저장하는 시스템을 설계할 수 있게 된다.
- **목표**
  1. 퀘스트/미션을 목표·조건·보상·상태로 데이터화하고 진행·완료를 추적한다.
  2. 대화(분기·조건·변수)를 데이터·그래프로 구성하고 게임 이벤트와 연결한다.
  3. 콘텐츠 진행 상태의 저장·복원과 라이브 추가(이벤트 퀘스트) 대응을 정리한다.

**핵심 용어**: 퀘스트 데이터(목표·조건·보상을 담은 정의), 대화 그래프(분기·조건으로 이어지는 대사 구조), 진행 상태(콘텐츠가 어디까지 완료됐는지 — 저장 대상).
...
|---|---|
| ScriptableObject architecture eBook | https://unity.com/resources/create-modular-game-architecture-with-scriptable-objects-ebook |
| Unity Visual Scripting(그래프 도구) | https://docs.unity3d.com/Packages/com.unity.visualscripting@latest |
| Yarn Spinner(대화, 커뮤니티) | https://docs.yarnspinner.dev/ |
| ink (inkle, 내러티브) | https://www.inklestudios.com/ink/ |

> Yarn Spinner·ink 등은 Unity 공식 패키지가 아니다. 도입 시 라이선스·버전을 별도로 확인한다.

**리서치 범위**

- **다룰 내용**: 퀘스트/미션 데이터·진행 추적, 대화 분기·변수, 진행 상태 저장·라이브 추가.
- **다루지 않을 내용**: 데이터 테이블 임포트(→ 49), 세이브 직렬화(→ 27), 현지화(→ 18), 스킬 시스템(→ 68).
- **연계**: 49. 게임 데이터 테이블 파이프라인, 27. 세이브 시스템과 직렬화

**산출물**: 데이터 기반 퀘스트·진행 추적 샘플, 분기 대화 시스템 예제, 콘텐츠 진행 저장·라이브 추가 정책 노트.

### 77. 게임 필·주스(Game Feel) 〔UI·입력·연출〕

**목적·목표**

- **목적**: 히트스톱·스크린셰이크·이징·피드백을 더해 입력에 대한 "손맛"을 체계적으로 다듬을 수 있게 된다.
- **목표**
  1. 타격·피격·획득 등 핵심 순간에 히트스톱(짧은 정지)·스크린셰이크·플래시를 더한다.
  2. 이징 곡선·예측 이동·관성으로 입력 반응을 매끄럽게 만든다.
  3. 과한 연출과 적절한 연출의 경계를 두고, 접근성(모션 민감)·성능 비용을 점검한다.

**핵심 용어**: 게임 필/주스(입력에 대한 시청각 피드백으로 만드는 손맛), 히트스톱(타격 순간 잠깐 멈춰 임팩트를 키우기), 이징(가속·감속 곡선으로 자연스러운 움직임).

**선행 학습 자료**

| 자료 | 출처 |
|---|---|
| Cinemachine(Impulse·셰이크) | https://docs.unity3d.com/Packages/com.unity.cinemachine@latest |
| AnimationCurve (이징) | https://docs.unity3d.com/ScriptReference/AnimationCurve.html |
| 『Game Feel』(Steve Swink) | https://www.gamefeelbook.com/ |

**리서치 범위**

- **다룰 내용**: 히트스톱·스크린셰이크·플래시, 이징·관성·예측 이동, 연출 강도·접근성·성능 점검.
- **다루지 않을 내용**: 카메라 구성 일반(→ 16), 트윈 UI 연출(→ 12), 파티클·VFX(→ 21), 접근성 일반(→ 59).
- **연계**: 16. Cinemachine과 Camera, 12. DOTween과 UI Animation

**산출물**: 히트스톱·셰이크·플래시 적용 전후 비교, 이징·관성 입력 반응 샘플, 연출 강도·접근성 점검 노트.
